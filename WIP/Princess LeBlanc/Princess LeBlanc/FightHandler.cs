...
namespace Princess_LeBlanc
{
    internal class FightHandler
    {
        ...
        internal static bool Wstatus = true;
        ...
        public static void LaneClear()
        {
            ...
            var useW = MenuHandler.LeBlancConfig.SubMenu("ClearL").Item("LaneClearW").GetValue<bool>() && SkillHandler.W.IsReady() && Wstatus == true;
        ...
        public static void JungleClear()
        {
            ...
            var useW = MenuHandler.LeBlancConfig.SubMenu("ClearJ").Item("JungleClearW").GetValue<bool>() && SkillHandler.W.IsReady() && Wstatus == true;
            var minions = MinionManager.GetMinions(
            ...
            if (Wstatus == false)
            {
                Utility.DelayAction.Add(100, () => SkillHandler.W.Cast(Player, PacketCast));
            }
        }
        ...
        public static void Flee()
        {
            ...
            if (MenuHandler.LeBlancConfig.Item("FleeW").GetValue<bool>() && Wstatus == true && SkillHandler.W.IsReady())
            {
        ...
        public static void Harass()
        {
            ...
            if (!useQ && useW && targetW && Wstatus == true)
            {
        ...
        public static void WLogic()
        {
            ...
            var countW = _wpos.CountEnemysInRange(400) <= MenuHandler.LeBlancConfig.SubMenu("Misc").SubMenu("backW").Item("SWcountEnemy").GetValue<Slider>().Value;
            ...
            var comboLast = MenuHandler.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.None && MenuHandler.LeBlancConfig.SubMenu("Misc").SubMenu("backW").Item("SWnoCombo").GetValue<bool>();

            if (!MenuHandler.LeBlancConfig.SubMenu("backW").Item("useSW").GetValue<bool>() || Wstatus == true)
            {
                return;
            }

            if (MenuHandler.Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && (playermana && t.HealthPercentage() > 8) || playerhealth || thp || dead || countW)
            {
        ...
        {
            var t = GetEnemy(SkillHandler.Q.Range, TargetSelector.DamageType.Magical);
        ...
        private static void ComboTanky()
        {
            var t = GetEnemy(SkillHandler.E.Range, TargetSelector.DamageType.Magical);
            var einrange = SkillHandler.E.IsInRange(t);
            var useE = SkillHandler.E.IsReady() && MenuHandler.LeBlancConfig.SubMenu("Combo").Item("useE").GetValue<bool>() && einrange;
            var useR = SkillHandler.R.IsReady() && MenuHandler.LeBlancConfig.SubMenu("Combo").Item("useR").GetValue<bool>() && einrange;
            var targetBuff = t.HasBuff("LeblancSoulShackle");
            ...
            if (!SkillHandler.E.IsReady() && !SkillHandler.R.IsReady())
            {
                Combo();
            }
        }
        ...
        {
            var t = GetEnemy(1200, TargetSelector.DamageType.Magical);
            ...
            else if (((t.Health - MathHandler.ComboDamage(t)) / t.MaxHealth) * 100 > (t.Health * 50) / 100 && tq)
            {
                ComboTanky();
            }
            else
